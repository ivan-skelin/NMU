...
cerestim.beginGroup()
cerestim.autoStim(1, 4);
cerestim.autoStim(2, 4);
cerestim.endGroup()
cerestim.wait(900);
cerestim.endSequence();
  eventString = 'StimSent140hz'; eventID = 11;
    [succeeded, ~] = NlxSendCommand(['-PostEvent "' char(eventString) '" ' int2str(eventID) ' 11']);

cerestim.play(30);
 eventString = 'StimEnd140hz'; eventID = 17;
    [succeeded, ~] = NlxSendCommand(['-PostEvent "' char(eventString) '" ' int2str(eventID) ' 11']);
...
fs=1000;
     ...
     %imagesc(flipud(zscore(spect_win_onset_avg,[],2)));
      ...
      title(sprintf('%s %s onset',dataBuffer.channelNames{ch_count+1}, 'CCEP 140hz'));
     ...
     imagesc(flipud(spect_win_offset_avg));xline(200,'w--','LineWidth',3)
       ...
       yticklabels({num2str(band_of_interest(end)),num2str(band_of_interest(1))});
       yticks([1 numel(wFreq)]); ylabel('freq (hz)')
         title(sprintf('%s %s offset',dataBuffer.channelNames{ch_count+1}, 'CCEP 140hz'));
