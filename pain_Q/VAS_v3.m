...
DrawFormattedText(window, 'What is your pain level?', 'center', windowRect(4) * 0.3, black);
...
% Adjust horizontal position for 'No pain' text
noPainPosition = numPositions(1) - numWidth / 2 - 50;  % 50 pixels to the left of the first number box
DrawFormattedText(window, 'No pain', noPainPosition, labelYpos, black, [], [], [], [], [], []);
DrawFormattedText(window, 'Worst pain', numPositions(end), labelYpos, black, [], [], [], [], [], []);
...
QuestionText = repmat({'Pain level'}, length(responses), 1);  % Cell array of questions
...
% Clean up and close
if ttl_flag == 1
    sendTTL_cpod(TTL_VAS_end);
    closeTTL_cpod(COM_label);
end

% Save individual results to a CSV file as usual
curr_time = datetime('now');
formattedTime = datestr(curr_time, 'HH:MM:SS');
title = convertCharsToStrings(sprintf('ExperimentResultsVAS_%s.csv',formattedTime));
title = strrep(title, ':', '-');
writetable(T, title);
